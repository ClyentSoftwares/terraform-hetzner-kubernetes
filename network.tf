locals {
  ...
  # Alias IP VIP — floats between control plane nodes for HA
...
resource "hcloud_network_subnet" "worker" {
  for_each = { for np in local.workers : np.name => np }
  ...
  ip_range = cidrsubnet(
    local.network_node_ipv4_cidr,
    local.network_node_ipv4_subnet_mask_size - split("/", local.network_node_ipv4_cidr)[1],
    2 + (local.network_node_ipv4_cidr_skip_first_subnet ? 1 : 0) + index(local.workers, each.value)
  )
}
