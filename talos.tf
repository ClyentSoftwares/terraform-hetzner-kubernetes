locals {
  ...
  talos_primary_endpoint = coalesce(local.talos_primary_node_public_ipv4, local.talos_primary_node_public_ipv6)
  talos_endpoints    = compact(local.control_plane_public_ipv4_list)

  # Kubernetes API
  kube_api_private_ipv4 = local.kube_api_load_balancer_private_ipv4
  ...
  # Talos Control (inline commands — no templatefiles)
  ...
  talosctl_retry_snippet = join(" ",
    [
      "[ \"$retry\" -gt 5 ] && exit 1 ||",
      "{ printf '%s\\n' \"Retry $retry/5 ...\"; retry=$((retry + 1)); sleep 10; }"
    ]
  )
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      if ${local.cluster_initialized}; then
        ...
        retry=1
        while ! ${local.talosctl_health_check_command} -n '${local.control_plane_private_ipv4_list[0]}'; do
          ...
          while true; do
            ...
            if [ "$current_version" = "${var.talos_version}" ] && [ "$current_schematic" = "${local.talos_schematic_id}" ]; then
              if [ "$retry" -gt 1 ]; then
                ...
                retry=1
                while ! ${local.talosctl_health_check_command} -n '${local.talos_primary_node_private_ipv4}'; do
                  ${local.talosctl_retry_snippet}
                done
...
resource "talos_machine_configuration_apply" "control_plane" {
  ...
  endpoint                    = coalesce(each.value.ipv4_address, each.value.ipv6_address)
...
resource "terraform_data" "talos_access_data" {
  input = {
    ...
    # Note: cluster_autoscaler_private_ipv4_list is excluded here to avoid a
    # dependency cycle through data.external.talos_member. Autoscaler node IPs
    # are discoverable only via Talos membership queries at runtime.
    worker_nodes = concat(local.worker_private_ipv4_list, local.dedicated_servers_talos_private_ipv4_list)
