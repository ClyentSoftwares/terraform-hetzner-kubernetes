locals {
  ...
  # Determine which architecture images are actually needed based on server types.
  # cax* = ARM64 (Ampere); everything else = AMD64.
...
resource "terraform_data" "amd64_image" {
  ...
  triggers_replace = [
    ...
    var.talos_version,
    local.talos_schematic_id,
    var.packer_amd64_builder.server_type,
...
resource "terraform_data" "arm64_image" {
  ...
  triggers_replace = [
    ...
    var.talos_version,
    local.talos_schematic_id,
    var.packer_arm64_builder.server_type,
